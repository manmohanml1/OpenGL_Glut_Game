					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Game Engine Lesson 02",fullscreenWidth,fullscreenHeight,256,fullscreen))
	{
	...
	{
	    if(!Scene->doneInitializing && Scene->level1)
        {
        ...
        if(!Scene->doneInitializing && Scene->level2)
        {
            if (!Scene->initGL())							// Initialize Our Newly Created GL Window
            {
                KillGLWindow();								// Reset The Display
        ...
        if(!Scene->doneInitializing && Scene->level3)
        {
            if (!Scene->initGL())							// Initialize Our Newly Created GL Window
            {
                KillGLWindow();								// Reset The Display
